<template>
    <section v-if="showSection" class="show-relation">
        <h1><a :href="'/' + word + '/' + rdata.name" @click="changeRelation(rdata.name,$event)">{{ relationName }}</a></h1>
        ...
        <div v-for="type in showType">
            <div v-if="relations[type].data.length">
                ...
                <ul class="list-inline">
                    ...
                    <a-word :relation="rel" :words="words" :show="infoType[type].a_word_show" ></a-word>
                    </li>
                </ul>
            </div>
        </div>
...
<script>
    ...
    export default{
        ...
        methods: {
            changeRelation(rel, event) {
                event.preventDefault();
                HUB.changeRelation(rel);
            }
        }
    }
</script>